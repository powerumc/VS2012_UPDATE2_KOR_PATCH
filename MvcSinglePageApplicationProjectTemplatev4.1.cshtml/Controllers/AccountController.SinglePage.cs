...
namespace $safeprojectname$.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private static void InitiateDatabaseForNewUser(string userName)
        {
            ...
            todoList.Title = "내 할 일 모음 #1";
            todoList.Todos = new List<TodoItem>();
            db.TodoLists.Add(todoList);
            db.SaveChanges();

            todoList.Todos.Add(new TodoItem() { Title = "할 일 #1", TodoListId = todoList.TodoListId, IsDone = false });
            todoList.Todos.Add(new TodoItem() { Title = "할 일 #2", TodoListId = todoList.TodoListId, IsDone = false });
            db.SaveChanges();
        }
        ...
        public ActionResult Disassociate(string provider, string providerUserId)
        {
            ...
            // 로그인한 사용자가 소유자인 경우에만 계정 연결 해제
            if (ownerAccount == User.Identity.Name)
            {
                // 사용자가 마지막 로그인 자격 증명을 삭제하지 못하도록 트랜잭션 사용
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? "암호를 변경했습니다."
                : message == ManageMessageId.SetPasswordSuccess ? "암호를 설정했습니다."
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            ...
            {
                // 사용자에게 로컬 암호가 없으므로 누락에 의해 발생한 모든 유효성 검사 오류를 제거합니다.
                // OldPassword 필드
        ...
        public ActionResult ExternalLoginCallback(string returnUrl)
        {
            ...
            {
                // 사용자가 로그인한 경우 새 계정 추가
                OAuthWebSecurity.CreateOrUpdateAccount(result.Provider, result.ProviderUserId, User.Identity.Name);
                return RedirectToLocal(returnUrl);
            }
            else
            {
                // 새로운 사용자이므로 원하는 멤버 자격 이름을 물어봄
